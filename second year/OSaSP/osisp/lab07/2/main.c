...
void print_record(int rec_no) {
    record_t record;
    // представление смещения в файле
...
void save_record(record_t *record, record_t *new_record, int rec_no) {
    ...
    lock.l_whence = SEEK_SET; // начальная позиция для блокировки
    lock.l_start = rec_no * sizeof(*record); // позиция блокировки по номеру записи
    lock.l_len = sizeof(*record);
    while (fcntl(fd, F_SETLK, &lock) == -1) { // установка блокировки
        if (errno == EAGAIN) {
    ...
    if (strcmp(current_record.name, record->name) != 0 || strcmp(current_record.address, record->address) != 0 || current_record.semester != record->semester) {
        ...
        lock.l_type = F_UNLCK; // разблокировка
