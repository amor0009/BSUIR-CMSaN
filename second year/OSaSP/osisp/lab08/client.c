...
int main(int argc, char const *argv[]) {
    ...
    // представления сетевых адресов в семействе протоколов TCP/IP
    ...
    server_address.sin_port = htons(port); // в порядок байтов сети
    ...
    struct dirent **namelist; // информация о записях в каталоге
    char lnk[1024], buffer[1024];
    mode_t link_to; // представление режима доступа к файлу или каталогам
    FILE *file;
    while (1) {
        memset(buffer, 0, sizeof(buffer));
    // точка перехода, проверяем значение флага и выполняем по значению
    check:
        if (flag) {
            printf("%s>", dir);
        ...
        if (strncasecmp(buffer, "CD ", strlen("CD ")) == 0) {
            ...
            // получение информации о кажом файле из сокета
