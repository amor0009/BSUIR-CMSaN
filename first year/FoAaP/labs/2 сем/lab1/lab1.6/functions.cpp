...
void ChangeFile(char* str, const char* file2, const char* file3)				// замена слов в тексте файла 1
{
	...
	char* text = (char*)calloc(80, sizeof(char));									
	char* word = (char*)calloc(80, sizeof(char));								// выделение памяти для слова из текста файла 1
	char* string = (char*)calloc(80, sizeof(char));								// выделение памяти для текста из файла 2
	char* sep = ",";															// указатель на символ разделитель
	int flag = 0;
	int j = 0;
	char* sn = '\n';
	char* separ = (char*)calloc(80, sizeof(char));								// выделение памяти для знака препинания или пробела из текста файла 1
	for (int i = 0; i < strlen(str); i++)										// цикл для прохода по строке
	{
		if (str[i] == ',' || str[i] == ';' || str[i] == ' ' || str[i] == '.')	// если сюда попали, значит слово для сравнения сформированно
		{
			...
			fgets(string, 80, f2);												// считываем текст с парами из файла 2 (f2.txt)
			...
			char* word = (char*)calloc(80, sizeof(char));
			free(separ);
			char* separ = (char*)calloc(80, sizeof(char));
