...
void FindProduct(struct s1* inf, struct s2** product, int n, int* number)		// поиск нужных полей структуры и их вывод
{
	...
	for (int i = 0; i < n; i++)
		if (strcmp(word, inf[i].name) == 0)										// сравниваем название продукта и ввод пользователя
		{
			flag = 1;
			num = num + 1;
			strcpy(prod[j].name, inf[i].name);									// копирование информации полей из первоначальной структуры в новую
			strcpy(prod[j].country, inf[i].country);
			prod[j].volume = inf[i].volume;
			j++;
		}
	prod = (struct s2*)realloc(prod, (num * sizeof(struct s2)));				// изменение динамической памяти
...
void StructFromFile2(struct s1** info, int n)									// чтение массива структур из бинароного файла
{
...
void StructInFile2(struct s1* info, int n)										// запись информации массива структур в бинарный файл
{
	FILE* fp = fopen("f.bin", "wb");
	fwrite(info, sizeof(struct s1), n, fp);
	/*for (int i = 0; i < n; i++)
	{
...
void StructInFile1(struct s2* product, int num)									// запись изменённого массива структур в бинарный файл
{
	FILE* fp = fopen("f1.bin", "wb");
	for (int i = 0; i < num; i++)
	{
		fwrite(product[i].name, sizeof(char), N, fp);
		fwrite(product[i].country, sizeof(char), N, fp);
		fwrite(&product[i].volume, sizeof(int), 1, fp);
	}
	fclose(fp);
}